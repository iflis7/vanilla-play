...
export const WebApi = async (): Promise<{ el: HTMLElement }> => {
    ...
    el.innerHTML = `
      ...
      <div class="mb-4 border-2 border-yellow-400 p-4 rounded-lg w-94 mx-auto">
            <h2 class="text-center text-orange-500 py-2"> 7. Websockets API</h2>
            <div class="flex justify-center w-full">
            <input id="websocket-input" type="text"  class="bg-gray-50 border border-green-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Type a message" required />
            <button id="websocket-send" type="button" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2 me-2 ml-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Send</button>
            </div>

            <div id="websocket-log" class="border border-y-teal-600 p-2 mt-4 "></div>
        </div>
    `;
    ...
    const initWebSocket = () => {
        ...
        const logContainer = el.querySelector('#websocket-log') as HTMLElement;
        const sendButton = el.querySelector('#websocket-send') as HTMLButtonElement;
        const input = el.querySelector('#websocket-input') as HTMLInputElement;
        ...
        ws.onmessage = (event) => {
          logContainer.innerHTML += `<p class="text-green-500">Received: <span class="text-xs text-white"> ${event.data} </span> </p>`;
        };
        ...
        sendButton.addEventListener('click', () => {
          ...
          ws.send(message);
          logContainer.innerHTML += `<p class="text-green-500">Sent: <span class="text-xs text-white"> ${message} </span> </p>`;
          input.value = '';
        });
      };
